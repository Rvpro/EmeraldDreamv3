...
class npc_dire_arena_commander : public CreatureScript
{
   public:
	   ...
	   bool OnGossipHello(Player * player, Creature * creature)
	   {
		   ...
		   if(isBattleActive)
			   player->ADD_GOSSIP_ITEM(GOSSIP_ICON_BATTLE, "К сожалению, битва уже идёт.", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+3);
		   else
		       player->ADD_GOSSIP_ITEM(GOSSIP_ICON_BATTLE, MSG_FIGHT_COMPUTER, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
		   player->ADD_GOSSIP_ITEM(GOSSIP_ICON_DOT, "Неважно.", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+2);
	   ...
	   bool OnGossipSelect(Player * player, Creature * creature, uint32 sender, uint32 actions)
	   {
		   ...
		   switch(actions)
		   {
		      case GOSSIP_ACTION_INFO_DEF+1:
				   ...
				   for(int i = 0; i < 3; i++)
				   {
					   if(player->HasSpell(spellDisables[i]))
						   player->removeSpell(spellDisables[i]);
				   }
		   ...
		   void UpdateAI(const uint32 diff)
		   {
			   ...
			   if(checkPlayer <= diff) // Checking battle as well
			   {
				   ...
				   if(hasLogged || !inZone)
				   {
						...
						m_PlayerGUID = NULL;
						playerName = "";
						sWorld->SendGlobalText("Участник испугался и бежал, кто следующий?", NULL);
				   }
					checkPlayer = 1000;
			   }
			   else
			   ...
			   while(uint32 eventIds = events.ExecuteEvent())
			   {
				   switch(eventIds)
				   {
					  ...
					  case EVENT_COMPLETED_WAVES:
						  me->MonsterYell("Поздравляем, прекрасный гладиатор прошел испытание, кто еще желает попробовать?", 
   ...
   public:
	   ...
	   struct npc_dire_maul_rb_guardAI : public ScriptedAI
	   {
		   ...
		   void Reset()
		   {
			   ...
			   me->MonsterYell("Ты не сможешь победить меня!", LANG_UNIVERSAL, me->GetGUID());
		   ...
		   void KilledUnit(Unit * who)
		   {
			   if(who && who->GetTypeId() != TYPEID_PLAYER)
				   return;
			   DoGivePlayerSpells(who->ToPlayer());
			   isBattleActive = false;
			   m_PlayerGUID = NULL;
			   playerName = "";
		   ...
		   void UpdateAI(const uint32 diff)
		   {
			   ...
			   {
				   me->MonsterYell("Слуги, на помощь!", LANG_UNIVERSAL, me->GetGUID());
...
class npc_dm_hank_the_tank : public CreatureScript
{
   public:
	   ...
	   struct npc_dm_hank_the_tankAI : public ScriptedAI
	   {
		   ...
		   void Reset()
		   {
			   me->MonsterYell("Вы уйдёте в тот мир!", LANG_UNIVERSAL, me->GetGUID());
			   summons.DespawnAll();
   ...
   public:
	   ...
	   struct npc_dm_field_medicAI : public ScriptedAI
	   {
		   ...
		   void UpdateAI(const uint32 diff)
		   {
			   ...
			   {
				   me->MonsterYell("Мастер мёртв??!! Я сдаюсь!", LANG_UNIVERSAL, me->GetGUID());
				   me->setFaction(35);
				   me->DespawnOrUnsummon(1);
				   showOnce = true;
				   return;
			   ...
			   if(uiLifebloomTimer <= diff)
			   {
				   if(canHealMaster)
				   {
					   DoCast(hank, SPELL_HEAL_LIFEBLOOM, true); 
					   casted++; 
					   if(casted != 3)
						   uiLifebloomTimer = 2000;
					   else 
...
class npc_dm_main_rogue : public CreatureScript
{
   public:
	   ...
	   struct npc_dm_main_rogueAI : public ScriptedAI
	   {
		   ...
		   void Reset()
		   {
			   me->MonsterYell("Что же, будет тебе урок!", LANG_UNIVERSAL, me->GetGUID());
			   InStealth = true;
			   InVanish = false;
			   comboPoints = 0;
		   ...
		   void UpdateAI(const uint32 diff)
		   {
			   ...
			   if(InVanish)
			   {
				   ...
				   {
					   me->CastSpell(me->getVictim(), SPELL_ROGUE_CHEAP_SHOT, true);
					   me->MonsterYell("В атаку!", LANG_UNIVERSAL, me->GetGUID());
					   InVanish = false;
				   }
				   else
					   uiWaitTimer -= diff;
...
class npc_dm_main_unholy : public CreatureScript
{
   public:
	   ...
	   struct npc_dm_main_unholyAI : public ScriptedAI
	   {
		   ...
		   void Reset()
		   {
			   me->MonsterSay("Что это? Ловушка?", LANG_UNIVERSAL, me->GetGUID());
			   uiCinematic = 9000;
			   cinematicPassed = 0;
			   checkGuin = true;
			   boneArmor = true;
		   ...
		   void UpdateAI(const uint32 diff)
		   {
			   ...
			   if(uiCinematic <= diff)
			   {
				   ...
				   {
					   me->MonsterSay("Ахахах, значит он желает драться? Серьезно?", LANG_UNIVERSAL, me->GetGUID());
					   cinematicPassed = 1;
					   uiCinematic = 10000;
				   }
				   else if(cinematicPassed == 1)
				   {
					   me->MonsterSay("Не будь дураком!", LANG_UNIVERSAL, me->GetGUID());
					   cinematicPassed = 2;
					   uiCinematic = 9000;
				   }
				   else if(cinematicPassed == 2)
				   {
					   me->MonsterSay("Брат, нет, пусть он сам. Мы просто посмотрим на эту схватку", LANG_UNIVERSAL, me->GetGUID());
					   cinematicPassed = 3;
				   }
			   }
			   else
			   ...
			   if(checkGuin)
			   {
				   if(guin && guin->isDead())
				   {
					   me->MonsterYell("Пусть это произойдёт!", LANG_UNIVERSAL, me->GetGUID());
	   ...
	   struct npc_dm_unholy_twinAI : public ScriptedAI
	   {
		   ...
		   void UpdateAI(const uint32 diff)
		   {
			   if(uiCinematic <= diff)
			   {
				   if (cinematicPassed == 0)
				   {
					   me->MonsterSay("Судя по всему он хочет драться с нами.", LANG_UNIVERSAL, me->GetGUID());
					   cinematicPassed = 1;
					   uiCinematic = 7000;
				   }
				   else if (cinematicPassed == 1)
				   {
					   me->MonsterSay("Да, мне тоже так кажется.", LANG_UNIVERSAL, me->GetGUID());
					   cinematicPassed = 2;
					   uiCinematic = 6000;
				   }
				   else if (cinematicPassed == 2)
				   {
					   me->MonsterSay("Что? Драться?", LANG_UNIVERSAL, me->GetGUID());
					   cinematicPassed = 3;
					   uiCinematic = 3000;
				   }
				   else if (cinematicPassed == 3)
				   {
					   me->MonsterYell("Неееееет!", LANG_UNIVERSAL, me->GetGUID());
					   cinematicPassed = 4;
				   }
			   }
			   else
	   ...
	   struct npc_dm_unholy_petAI : public ScriptedAI
	   {
		   ...
		   void UpdateAI(const uint32 diff)
		   {
			   if(uiCinematicTimer <= diff)
			   {
				   if (cinematicPassed == 0)
				   {
					   me->MonsterSay("Почему нам нельзя драться, мастер?", LANG_UNIVERSAL, me->GetGUID());
					   cinematicPassed = 1;
					   uiCinematicTimer = 5000;
				   }
				   else if (cinematicPassed == 1)
				   {
					   me->MonsterYell("Потому что я голоден!", LANG_UNIVERSAL, me->GetGUID());
					   me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_ATTACKABLE_1 | UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_NOT_SELECTABLE);
					   me->SetReactState(REACT_AGGRESSIVE);
   ...
   public:
	   ...
	   struct npc_dm_rider_guardianAI : public ScriptedAI
	   {
		   ...
		   void UpdateAI(const uint32 diff)
		   {
			   ...
			   {
				   me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_ATTACKABLE_1 | UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_NOT_SELECTABLE);
				   me->MonsterYell("Я...Я сдаюсь!", LANG_UNIVERSAL, me->GetGUID());
			   ...
			   if(m_Phase == PHASE_START_COMBAT)
			   {
				   ...
				   if(IsInFlight)
				   {
					   if(uiFlameStrikeTimer <= diff)
					   {
						   ...
						   {
							   me->MonsterTextEmote("Дракон Арены имеет очень много огня внутри!", me->getVictim()->GetGUID(), true);
   ...
   public:
	   ...
	   void OnUpdateZone(Player * player, uint32 zone, uint32 area)
	   {
		   ...
		   if(isBattleActive && player->GetGUID() != m_PlayerGUID)
		   {
			   ...
			   ChatHandler(player).SendSysMessage("Вам нельзя здесь находиться, пока идёт битва!");
		   }
	   }
