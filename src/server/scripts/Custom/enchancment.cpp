...
void Enchant(Player* player, Item* item, uint32 enchantid)
{
    if (!item)
    {
        player->GetSession()->SendNotification("Сначала вы должны одеть предмет, на который хотите наложить чары!");
        return;
    }
    ...
    player->GetSession()->SendNotification("|cff800080%s |cffFF0000успешно зачарован!", item->GetTemplate()->Name1.c_str());
}
...
void RemoveEnchant(Player* player, Item* item)
{
	if (!item)
	{
		player->GetSession()->SendNotification("У вас не одет предмет?");
		return;
	}

	item->ClearEnchantment(PERM_ENCHANTMENT_SLOT);
	player->GetSession()->SendNotification("|cff800080%s's |cffFF0000наложение чар убрано!", item->GetTemplate()->Name1.c_str());
}
    ...
    npc_enchantment() : CreatureScript("npc_enchantment") { }
        ...
        bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action)
        {
            ...
            switch (action)
            {
                ...
                case 2: // Enchant 2H Weapon
					{
						...
						{
							player->GetSession()->SendNotification("У вас не одето двуручное оружие, но здесь чары которые вы можете использовать.");
							player->PlayerTalkClass->SendCloseGossip();
						}
					}
					break;
                ...
                case 4: // Enchant Head
                    player->ADD_GOSSIP_ITEM(1, "30 spellpower, 10mp5", GOSSIP_SENDER_MAIN, 124);
                    player->ADD_GOSSIP_ITEM(1, "30 spellpower, 20crit rating", GOSSIP_SENDER_MAIN, 125);
                    player->ADD_GOSSIP_ITEM(1, "29 spellpower, 20resilience", GOSSIP_SENDER_MAIN, 126);
                    player->ADD_GOSSIP_ITEM(1, "30 stamina, 25resilience", GOSSIP_SENDER_MAIN, 127);
                    player->ADD_GOSSIP_ITEM(1, "37 stamina, 20defence rating", GOSSIP_SENDER_MAIN, 128);
                    player->ADD_GOSSIP_ITEM(1, "50 attack power, 20crit rating", GOSSIP_SENDER_MAIN, 129);
                    player->ADD_GOSSIP_ITEM(1, "50 attack power, 20resilience", GOSSIP_SENDER_MAIN, 130);
					player->ADD_GOSSIP_ITEM(1, "25 arcane resist, 30stamina", GOSSIP_SENDER_MAIN, 131);
					player->ADD_GOSSIP_ITEM(1, "25 fire resist, 30stamina", GOSSIP_SENDER_MAIN, 132);
					player->ADD_GOSSIP_ITEM(1, "25 shadow resist, 30stamina", GOSSIP_SENDER_MAIN, 133);
					player->ADD_GOSSIP_ITEM(1, "25 frost resist, 30stamina", GOSSIP_SENDER_MAIN, 134);
					player->ADD_GOSSIP_ITEM(1, "25 nature resist, 30stamina", GOSSIP_SENDER_MAIN, 135);
					player->ADD_GOSSIP_ITEM(1, "<-Back", GOSSIP_SENDER_MAIN, 300);
                ...
                case 5: // Enchant Shoulders
                    ...
                    {
                        player->ADD_GOSSIP_ITEM(1, "120 attackpower, 15crit rating", GOSSIP_SENDER_MAIN, 136);
                        player->ADD_GOSSIP_ITEM(1, "70 spellpower, 8mp5", GOSSIP_SENDER_MAIN, 137);
                        player->ADD_GOSSIP_ITEM(1, "60 dodge rating, 15defense rating", GOSSIP_SENDER_MAIN, 138);
                        player->ADD_GOSSIP_ITEM(1, "70 spellpower, 15crit rating", GOSSIP_SENDER_MAIN, 139);
                    }
                    player->ADD_GOSSIP_ITEM(1, "40 attackpower, 15crit rating", GOSSIP_SENDER_MAIN, 140);
                    player->ADD_GOSSIP_ITEM(1, "24 spellpower, 8mp5", GOSSIP_SENDER_MAIN, 141);
                    player->ADD_GOSSIP_ITEM(1, "20 dodge rating, 15defence rating", GOSSIP_SENDER_MAIN, 142);
                    player->ADD_GOSSIP_ITEM(1, "30 stamina, 15 resilience", GOSSIP_SENDER_MAIN, 143);
                    player->ADD_GOSSIP_ITEM(1, "24 spellpower, 15crit rating", GOSSIP_SENDER_MAIN, 144);
                    player->ADD_GOSSIP_ITEM(1, "23 spellpower, 15resilience", GOSSIP_SENDER_MAIN, 145);
                    player->ADD_GOSSIP_ITEM(1, "40 attackpower, 15resilience", GOSSIP_SENDER_MAIN, 146);
					player->ADD_GOSSIP_ITEM(1, "<-Back", GOSSIP_SENDER_MAIN, 300);
				...
				case 10: //Enchant legs
					...
					player->ADD_GOSSIP_ITEM(1, "75 attackpower, 22 crit rating", GOSSIP_SENDER_MAIN, 186);
					player->ADD_GOSSIP_ITEM(1, "50 spellpower, 20 spirit", GOSSIP_SENDER_MAIN, 187);
					player->ADD_GOSSIP_ITEM(1, "50 spellpower, 30 stamina", GOSSIP_SENDER_MAIN, 188);
					player->ADD_GOSSIP_ITEM(1, "<-Back", GOSSIP_SENDER_MAIN, 300);
				...
				case 13: //Enchant Off-Hand weapons
					{
						...
						if(!item)
						{
							player->GetSession()->SendNotification("Это наложение чар требует одетое одноручное оружие в левой руке.");
							player->PlayerTalkClass->SendCloseGossip();
							return false;
						}
						...
						{
							player->GetSession()->SendNotification("В твоей левой руке нету оружия.");
							player->PlayerTalkClass->SendCloseGossip();
						}
					}
					break;
				...
				case 113:
					{
						...
						if (!item)
						{
							player->GetSession()->SendAreaTriggerMessage("Наложение чар требует 2ручное оружие!");
							player->PlayerTalkClass->SendCloseGossip();
							return false;
						}
						...
						{
							player->GetSession()->SendAreaTriggerMessage("У тебя не одето двуручного оружия.");
							player->PlayerTalkClass->SendCloseGossip();
						}
					}
                    break;
				...
				case 118:
					{
						...
						if(!item)
						{
							player->GetSession()->SendAreaTriggerMessage("Для этого наложения чар необходимо одеть щит.");
							player->PlayerTalkClass->SendCloseGossip();
							return false;
						}
				...
				case 205:
					{
						...
						if(!item)
						{
							player->GetSession()->SendAreaTriggerMessage("Это наложение чар требует 1ручное оружие в левой руке.");
							player->PlayerTalkClass->SendCloseGossip();
							return false;
						}
