                ...
                case SPELLFAMILY_DRUID:
                {
                    ...
                    if (aura->GetSpellInfo()->SpellFamilyFlags[1] & 0x10)
                    {
                        if (AuraEffect const* aurEff = aura->GetEffect(EFFECT_1))
                        {
                            ...
                            if (Unit* caster = aura->GetCaster())
                            {
                                int32 mana = aura->GetSpellInfo()->CalcPowerCost(caster, aura->GetSpellInfo()->GetSchoolMask()) * chargesRemoved / 2;
    ...
    switch (dummySpell->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_DRUID:
        {
            ...
            else if (dummySpell->SpellIconID == 2850)
            {
                ...
                if (effIndex == 0)
                {
                   if (!(procSpell->SpellFamilyFlags[0] & 0x04400) || procSpell->SpellIconID != 961)
                        return false;
